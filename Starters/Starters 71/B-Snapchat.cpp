/**
 * CODER=> xavierbeast68
 * Problem : Snapchat -> Codechef Starters 71
 * Date : 28-12-2022
 * URL : https://www.codechef.com/START71C/problems/SNAPCHAT
...
void solve()
{
	/*--Let's Code--*/
	read(n);
	vector<ll> a = readvector(n);
	vector<ll> b = readvector(n);

	int maxCount = LONG_LONG_MIN, count = 0;
	FOR(i, 0, n)
	{
		count++;
		if (a[i] == 0 || b[i] == 0)
