/**
 * CODER=> xavierbeast68
 * Problem : Convert to permutation -> Codechef Starters 69
 * Date : 14-12-2022
 * URL : https://www.codechef.com/START69C/problems/PERMUTATION
...
void solve()
{
    ...
    if(vec[0]>0)
    {
        while(vec[0]>0)
        {
            int pos = find(begin(vec)+1,vec.end(),0) - vec.begin();
            if(pos < (n+1) )
            {
                vec[pos]=1;
                vec[0]--;
    ...
    FOR(i,1,n+1)
    {
        ...
        if(vec[i]>1)
        {
            while(vec[i]>1)
            {
                int pos = find(begin(vec)+(i+1),vec.end(),0) - vec.begin();
                if(pos < (n+1) )
                {
                    vec[pos]=1;
                    vec[i]--;
