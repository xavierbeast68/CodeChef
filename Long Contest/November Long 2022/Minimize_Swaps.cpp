/**
 * CODER=> xavierbeast68
 * Problem : Minimize Swaps -> Codechef November Long 2022
 * Date : 14-11-2022
 * URL : https://www.codechef.com/NOV221C/problems/DIVBY3
...
bool bitAtGivenPosSetOrUnset(uint32_t n, int k)
{
    int New_num = 1 << (k - 1);
...
void solve()
{
    /*--Let's Code--*/
    string s; cin >> s;
    uint32_t num = stoll(s);
    int ev = 0, od = 0;
    for (int i = 1; i <= 32; i++)
    {
        if (i % 2 == 1 && bitAtGivenPosSetOrUnset(num, i))
        {
            od++;
        }
        if (i % 2 == 0 && bitAtGivenPosSetOrUnset(num, i))
        {
            ev++;
        }
    }
