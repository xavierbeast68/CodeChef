/**
 * CODER=> xavierbeast68
 * Problem : Splitting Balls -> Codechef November Long 2022
 * Date : 14-11-2022
 * URL : https://www.codechef.com/NOV221C/problems/SPBALL
...
{   //println("##");
    ...
    {
        dp[num] = ((num % MOD) * ((facti(num - 1, dp)) % MOD)) % MOD;
        return dp[num];
    }
}
...
void solve()
{
    ...
    int maxi = maxOf(arr);
    vi dp(maxi + 1);
    int maxii = facti(maxi, dp);
    int fsum = 0;
    FOR(i, 0, n)
    {
        fsum = ((fsum % MOD) + (dp[arr[i]]) % MOD) % MOD;
    }
    println(fsum);
    */
    read(n); int maxi = 0;
    ...
    FOR(i, 0, n)
    {
        ...
        {
            maxi = x;
            fsum = ((fsum % MOD) + (facti(maxi, dp)) % MOD) % MOD;
        }
        else
        {
            fsum = ((fsum % MOD) + (dp[x]) % MOD) % MOD;
        }
