/**
 * CODER=> xavierbeast68
 * Problem : Unrealistic_Gift -> Code Smasher Coding Contest (Codechef)
 * Date : 13-11-2022
 * URL : https://www.codechef.com/CDSM2022/problems/URGFT
		...
		for (int k = 0; k < i; k++)
		{
			for (int j = 0; j <= 10; j++)
			{
				...
				else
				{
					...
					{
						string s1 = s, s2 = s;
						s1 = s1 + to_string(j) + to_string(l);
						s2 = to_string(j) + to_string(l) + s2;
						int x1 = stoi(s1);
						int x2 = stoi(s2);
						if (x1 % 9 == 0) {a.pb(x1);}
						if (x2 % 9 == 0) {a.pb(x2);}
